...
String configPage() {
    ...
    page += F("<!doctype html><html lang='zh-CN'><head><meta charset='utf-8'>"
              ...
              "button{width:100%;height:44px;margin-top:18px;border:0;border-radius:5px;background:var(--accent);color:white;font-weight:700;font-size:15px}"
    ...
    page += String(offline);
    page += F("'></div></div></section><button type='submit'>保存并重启</button></form>"
              "<p class='hint'>设备端：左右切换节点，上下切换页面，A 刷新，长按 B 重新开启配置热点，A+B 长按重启。</p>"
              "</main></body></html>");
    return page;
}
