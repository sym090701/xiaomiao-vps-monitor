...
cp "$BUILD_DIR/firmware.bin" "$OUTPUT_DIR/XiaoMiao-VPS-Monitor-v1.1.0-Launcher.bin"
"$PYTHON" "$ESPTOOL" --chip esp32 merge_bin \
    --output "$OUTPUT_DIR/XiaoMiao-VPS-Monitor-v1.1.0-Full-4MB.bin" \
...
if command -v sha256sum >/dev/null 2>&1; then
    sha256sum XiaoMiao-VPS-Monitor-v1.1.0-Launcher.bin \
              XiaoMiao-VPS-Monitor-v1.1.0-Full-4MB.bin > SHA256SUMS
else
    shasum -a 256 XiaoMiao-VPS-Monitor-v1.1.0-Launcher.bin \
                  XiaoMiao-VPS-Monitor-v1.1.0-Full-4MB.bin > SHA256SUMS
fi
